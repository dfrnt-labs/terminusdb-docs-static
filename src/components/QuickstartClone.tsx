    ...
    <div className="my-6 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-4">
      ...
      {state === "idle" && (
        <div className="space-y-2">
          ...
          <button
            onClick={handleClone}
            className="w-full rounded-md bg-sky-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 transition-colors"
            aria-label={label}
          >
            {label}
          </button>
      ...
      {state === "loading" && (
        <button
          disabled
          className="w-full rounded-md bg-sky-600/70 px-4 py-2.5 text-sm font-medium text-white shadow-sm cursor-not-allowed flex items-center justify-center gap-2"
          aria-busy="true"
      ...
      {state === "success" && (
        <div className="space-y-2">
          <button
            disabled
            className="w-full rounded-md bg-emerald-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm cursor-default flex items-center justify-center gap-2"
            aria-label="Database cloned successfully"
          >
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300">
            ...
            <a href="#step-4-create-a-branch" className="font-medium text-sky-600 dark:text-sky-400 hover:underline">
              Step 4 — branching
            </a>
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-2 text-amber-700 dark:text-amber-400">
            ...
            <span className="text-sm font-medium">
              TerminusDB is not running on {settings.serverUrl.replace(/^https?:\/\//, "")}
            </span>
          </div>
          <div className="rounded-md bg-slate-900 p-3">
            <p className="mb-1 text-xs text-slate-400">Start it with:</p>
...
{`docker run -d --name terminusdb -p 127.0.0.1:6363:6363 \\
  ...
  terminusdb/terminusdb-server:v12`}
          ...
          <button
            onClick={handleRetry}
            className="w-full rounded-md bg-sky-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 transition-colors"
          >
            Try again
          </button>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-2 text-red-700 dark:text-red-400">
            ...
            <span className="text-sm">
              Authentication failed. Check your credentials in the connection settings (gear icon).
            </span>
          </div>
          <button
            onClick={handleRetry}
      ...
      {state === "error-exists" && (
        <div className="flex items-start gap-2 text-slate-600 dark:text-slate-300">
          <svg className="mt-0.5 h-4 w-4 flex-shrink-0 text-slate-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      ...
      {state === "error-other" && (
        <div className="space-y-3">
          <div className="flex items-start gap-2 text-red-700 dark:text-red-400">
            ...
            <span className="text-sm">
              Clone failed: {errorDetail}.{" "}
              <a href="/docs/troubleshooting-connection" className="font-medium text-sky-600 dark:text-sky-400 hover:underline">
                Troubleshooting →
              </a>
            </span>
          </div>
